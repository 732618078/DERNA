...
setwd("F:\\biology\\TCGA\\STAD\\01Counts\\05Analysis\\01Deversity\\miRNA")                    #设置工作目录
library("edgeR")
...
group=c(rep("normal",45),rep("tumor",446))                         #按照癌症和正常样品数目修改
...
#xMax=300	#解决PValue=0的情况
xMax=max(-log10(allDiff$FDR))+1
yMax=12
plot(-log10(allDiff$FDR), allDiff$logFC, xlab="-log10(FDR)",ylab="logFC",
     main="miRNA_Volcano", xlim=c(0,xMax),ylim=c(-yMax,yMax),yaxs="i",pch=20, cex=0.4)
